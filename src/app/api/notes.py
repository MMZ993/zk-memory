...
def create_link_endpoint(body: LinkCreate, db: Session = Depends(get_db)):
    return create_link(db, body.model_dump())


@router.delete("/links/{link_id}", status_code=204)
def delete_link_endpoint(link_id: str, db: Session = Depends(get_db)):
...
def add_tag_endpoint(note_id: str, body: TagCreate, db: Session = Depends(get_db)):
    if not get_note(db, note_id):
        raise HTTPException(status_code=404, detail="Note not found")
